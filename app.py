...
def css():
    st.markdown(
        """
<style>
.block-container { padding-top: 0.7rem; padding-left: 0.7rem; padding-right: 0.7rem; max-width: 900px; }
...
def go(page: str):
    ...
    if dest:
        st.session_state.pending_deck = dest
...
def render_dual_views(stage: dict, fl: dict):
    dist = max(0.1, total_nm(stage))
    done = min(fl["nm_done"], dist)
    st.caption("Top-down track")
    acc = 0.0
    marks = []
    for i, icao in enumerate(stage["route"]):
        ...
        here = abs(acc - done) < max(8.0, dist * 0.03)
        marks.append(f"{'▶' if here else '•'} {name} ({acc:.0f} NM)")
    st.write(" · ".join(marks) if len(marks) < 5 else " | ".join(marks))
    st.progress(min(1.0, done / dist))
    st.caption("Side profile — altitude vs distance")
    xs = fl.get("path_nm") or [0.0]
    ys = fl.get("path_alt") or [fl.get("alt", 0)]
    # downsample so phones do not choke
    step = max(1, len(xs) // 40)
    st.line_chart(ys[::step] + [ys[-1]], width='stretch', height=220)
...
def page_fly():
    ...
    with right:
        r1 = st.columns(3)
        r1[0].metric("TAS", f"{fl['tas']:.0f} kt")
        r1[1].metric("ALT", f"{fl['alt']:.0f} ft")
        r1[2].metric("FUEL", f"{max(0, fl['fuel']):.1f} gal")
        r2 = st.columns(2)
        r2[0].metric("DONE", f"{fl['nm_done']:.1f} / {total_nm(s):.1f} NM")
        r2[1].metric("WIND", f"{fl['wind']:.0f} kt")
        render_dual_views(s, fl)
...
def page_debrief():
    ...
    if res.get("pass"):
        ...
        st.warning("Not yet. Read the table. Fly it again.")
...
def page_lab():
    ...
    labels = {s["id"]: f"Stage {s['id']} — {s['title']}" for s in STAGES}
    choice = st.selectbox(
        "Lab for this stage",
        open_ids,
    ...
    ac = AIRCRAFT[s["aircraft"]]
    truth = true_plan(s)
    st.markdown(f"**{s['title']}** · {s['theater']}")
    st.write(s["teach"])
    st.code(s["formula"])

    if s["id"] == 1:
        st.write("Move the sliders. The bigger bar is the force that is winning.")
        c1, c2 = st.columns(2)
        thrust_in = c1.slider("Thrust", 0, 100, 60, key="lab1_thrust")
        mass_in = c1.slider("Weight (mass)", 20, 100, 50, key="lab1_mass")
        flaps_in = c2.slider("Flaps", 0, 40, 0, key="lab1_flaps")
        speed_in = c2.slider("Speed", 40, 120, 85, key="lab1_speed")
        lift = 0.4 * speed_in + flaps_in * 0.8
        weight = mass_in * 1.1
        drag = 18 + flaps_in * 0.5 + max(0, speed_in - 70) * 0.2
        st.bar_chart({"Lift": lift, "Weight": weight, "Thrust": thrust_in, "Drag": drag})
        if lift > weight and thrust_in > drag:
            st.success("Climbing and speeding up.")
        elif lift > weight:
            st.info("Climbing, but drag is eating speed.")
        elif thrust_in > drag:
            st.info("Speeding up, not climbing.")
        else:
            st.warning("Sinking and slowing. Lower the nose.")
        st.caption("Lift is Bernoulli and Newton together. We do not use the equal-transit-time story.")

    elif s["id"] == 2:
        st.write("Pequet's hop is 5.2 NM. Pick a speed. Time is distance ÷ speed.")
        speed = st.slider("Speed (kt)", 30, 90, int(ac["cruise_kt"]), key="lab2_spd")
        hours = 5.2 / max(1.0, speed)
        st.metric("ETE", f"{hours * 60:.1f} min")
        st.write(f"5.2 NM ÷ {speed} kt × 60 = {hours * 60:.1f} minutes.")

    elif s["id"] == 3:
        st.write("Headwind lowers groundspeed. Fuel must cover the slower hours plus reserve.")
        tas = st.slider("TAS (kt)", 60, 120, int(ac["cruise_kt"]), key="lab3_tas")
        wind = st.slider("Headwind (kt)", 0, 40, int(s["wind_kt"]), key="lab3_wind")
        gs = max(40.0, tas - wind)
        hours = truth["dist_nm"] / gs
        fuel = ac["burn_gph"] * hours + ac["reserve_gal"]
        a, b, c = st.columns(3)
        a.metric("Groundspeed", f"{gs:.0f} kt")
        b.metric("ETE", f"{hours * 60:.0f} min")
        c.metric("Fuel needed", f"{fuel:.1f} gal")
        st.caption(f"burn {ac['burn_gph']} gph × {hours:.2f} h + reserve {ac['reserve_gal']} gal")

    elif s["id"] == 4:
        st.write("Add the legs. Total time uses the sum, not one city pair.")
        d1, d2 = s["legs_nm"]
        st.write(f"Delhi–Allahabad {d1} NM. Allahabad–Kolkata {d2} NM.")
        gs = st.slider("Groundspeed (kt)", 60, 140, int(truth["gs_kt"]), key="lab4_gs")
        total = d1 + d2
        st.metric("Total distance", f"{total:.1f} NM")
        st.metric("Total ETE", f"{(total / gs) * 60:.0f} min")

    elif s["id"] == 5:
        st.write("High and hot: the same indicated speed gives less lift. Nose-up at low speed stalls you.")
        elev = st.slider("Field elevation (ft)", 0, 8000, 4390, key="lab5_elev")
        heat = st.slider("Heat", 0, 40, 25, key="lab5_heat")
        pitch = st.slider("Nose up (deg)", 0, 16, 4, key="lab5_pitch")
        thin = 1.0 - elev / 40000 - heat / 400
        lift = 90 * thin * (1 + pitch * 0.04)
        weight = 100
        st.metric("Relative lift", f"{lift:.0f}")
        st.metric("Weight", f"{weight:.0f}")
        if pitch > 10 and thin < 0.85:
            st.warning("Stall. Lower the nose. Thin air does not care that you pulled.")
        elif lift > weight:
            st.success("You can climb, slowly.")
        else:
            st.info("Not enough lift. More speed, less pull.")

    elif s["id"] == 6:
        st.write("Stay 3 NM behind. Time to the field is distance ÷ speed.")
        lead_nm = st.slider("Lead distance to field (NM)", 4, 30, 8, key="lab6_lead")
        lead_kt = st.slider("Lead speed (kt)", 80, 180, 140, key="lab6_ls")
        you_nm = st.slider("Your distance (NM)", 4, 30, 8, key="lab6_you")
        you_kt = st.slider("Your speed (kt)", 80, 180, 140, key="lab6_ys")
        t_lead = lead_nm / lead_kt * 60
        t_you = you_nm / you_kt * 60
        gap = t_you - t_lead
        spacing = abs(gap) / 60 * (lead_kt if gap > 0 else you_kt)
        st.metric("Spacing at the field", f"{spacing:.1f} NM")
        if spacing >= 3:
            st.success("Legal. 3 NM or more.")
        else:
            st.warning("Too close. Slow down.")

    elif s["id"] == 7:
        st.write("Each border can change the wind. Recompute groundspeed. Do not keep the first plan.")
        tas = st.slider("TAS (kt)", 80, 180, int(ac["cruise_kt"]), key="lab7_tas")
        w1 = st.slider("Headwind Cologne–Nuremberg", 0, 40, 10, key="lab7_w1")
        w2 = st.slider("Headwind Nuremberg–Vienna", 0, 40, 20, key="lab7_w2")
        w3 = st.slider("Headwind Vienna–Budapest (− is tail)", -20, 40, -10, key="lab7_w3")
        legs = s["legs_nm"]
        hours = legs[0] / max(40, tas - w1) + legs[1] / max(40, tas - w2) + legs[2] / max(40, tas - w3)
        st.metric("New ETE", f"{hours * 60:.0f} min")
        st.caption("If you fly the first-leg wind the whole way, the fuel math is a lie.")

    elif s["id"] == 8:
        st.write("Endurance is the fuel you may burn, not the fuel in the tanks.")
        burn = st.slider("Burn now (gph)", 50, 120, int(ac["burn_gph"]), key="lab8_burn")
        fuel = st.slider("Fuel on board (gal)", 80, int(ac["fuel_cap_gal"]), 300, key="lab8_fuel")
        endu = max(0.0, (fuel - ac["reserve_gal"]) / burn)
        st.metric("Hours you may still fly", f"{endu:.2f} h")
        st.write(f"({fuel} − reserve {ac['reserve_gal']}) ÷ {burn} gph.")
        if endu * ac["cruise_kt"] < 323:
            st.warning("Not enough to reach Athens from Brindisi without the reserve. Land Brindisi.")
        else:
            st.success("Athens is inside endurance if the leak does not get worse.")

    elif s["id"] == 9:
        st.write("Point of no return: when fuel to go equals fuel to return, you have already chosen.")
        hold = st.slider("Minutes already held", 0, 40, 18, key="lab9_hold")
        burn_mult = st.slider("Burn multiplier", 100, 140, 125, key="lab9_bm") / 100
        left_min = 40 - hold
        fuel_flow = ac["burn_gph"] * burn_mult
        st.metric("Hold left", f"{left_min} min")
        st.metric("Fuel flow now", f"{fuel_flow:.0f} gph")
        st.write("If roughness started at minute 18, turning back spends less fuel than finishing 22 more minutes.")
        if hold >= 18 and burn_mult >= 1.2:
            st.warning("Turn back is the clean number. Pride is not a reserve.")
        else:
            st.info("You can finish the hold if burn stays at book value.")

    else:
        st.write("Stack the legs. London UTC+0 → Athens +2 → Karachi +5 → India +5:30.")
        gs = st.slider("Average GS (kt)", 300, 500, 420, key="lab10_gs")
        dist = truth["dist_nm"]
        hours = dist / gs
        st.metric("Flight time", f"{hours:.1f} h")
        st.write("Depart London 08:00 UTC. Add 5 hours 30 minutes at Dum Dum for the clock on the wall.")
        arrive_utc_h = 8 + hours
        ist_h = arrive_utc_h + 5.5
        st.metric("Clock at Dum Dum (IST)", f"{int(ist_h) % 24:02d}:{int((ist_h % 1) * 60):02d}")
        st.caption("Corridor Athens–Karachi is overflight. No invented field.")

    st.divider()
    if st.button("Back to this stage briefing"):
        st.session_state.active = s["id"]
        go("brief")
        st.rerun()
...
def page_atc():
    st.title("ATC Tower")
    st.write("Two aircraft, one merge, 3 NM minimum. Same rule as Stage 6 and Stage 10.")
    lead_nm = st.slider("Lead aircraft distance to merge (NM)", 5, 40, 20, key="atc_lead_nm")
    lead_kt = st.slider("Lead speed (kt)", 80, 250, 140, key="atc_lead_kt")
    you_nm = st.slider("You distance to merge (NM)", 5, 40, 28, key="atc_you_nm")
    you_kt = st.slider("Your speed (kt)", 80, 250, 140, key="atc_you_kt")
    t_lead = lead_nm / lead_kt * 60
    t_you = you_nm / you_kt * 60
    ...
    if spacing_nm >= 3:
        ...
        st.warning("Conflict. Slow, or take a longer route.")
...
with st.sidebar:
    ...
    nav = st.radio(
        "Deck",
        ["Hub", "Brief / fly", "Atlas", "Flight Lab", "ATC Tower"],
        label_visibility="collapsed",
        key="deck",
    )
    st.markdown(f"Rank **{rank_for(st.session_state.unlocked)}**")
    st.caption(f"Active stage {st.session_state.active}")
